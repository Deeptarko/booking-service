...
@SpringBootTest(properties = {"spring.jpa.hibernate.ddl-auto=create", "spring.jpa.show-sql=true"})
@Testcontainers
class BookingServiceIntegrationTest {
    ...
    void setup() {
        ...
        inventoryRepository.save(Inventory.builder().hotelId(1L).roomTypeId(101L).date(LocalDate.of(2026, 1, 20)).availableRooms(5).build());
    }
    ...
    void shouldPreventDoubleBooking() throws Exception {
        ...
        Callable<Boolean> task = () -> {
            try {
                latch.await();
                bookingService.reserveRooms(1L, 101L, LocalDate.of(2026, 1, 20), LocalDate.of(2026, 1, 21), 4);
                return true;
            } catch (Exception e) {
                return false;
            }
